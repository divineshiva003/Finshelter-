...
// 	try {
// 		// Fetch all services with all fields including packages and processingDays
// 		const services = await Service.find({});
// 		res.json({ services });
// 	} catch (err) {
...
// 		// First, clean up invalid services for all customers
// 		const Order = require('../models/orderModel');
// 		const allOrders = await Order.find({});
// 		const validOrderIds = new Set(allOrders.map(order => order.orderId));
		
// 		// Get all customers
// 		const allCustomers = await User.find({ role: "customer" });
		
// 		for (const customer of allCustomers) {
// 			const originalServicesCount = customer.services.length;
			
// 			// Remove services that don't have corresponding orders in database
// 			customer.services = customer.services.filter(service => {
// 				const hasValidOrder = service.orderId && validOrderIds.has(service.orderId);
// 				if (!hasValidOrder && service.orderId) {
// 					console.log(`Removing invalid service with orderId: ${service.orderId} for customer: ${customer.name}`);
// 				}
// 				return hasValidOrder;
// 			});
			
// 			// Save if services were cleaned up
// 			if (customer.services.length !== originalServicesCount) {
// 				await customer.save();
// 				console.log(`Cleaned ${originalServicesCount - customer.services.length} invalid services for customer: ${customer.name}`);
// 			}
// 		}
...
// 		// Send welcome email using Nodemailer (Gmail)
// 		try {
// 			await sendGmail({
// 				to: email,
// 				subject: "Welcome to Finshelter Team",
// 				html: `
// 					<!DOCTYPE html>
// 					<html>
// 					<head>
...
// 						<div class="container">
// 							<div class="header">
// 								<h2>Welcome to Finshelter!</h2>
// 							</div>
// 							<div class="content">
// 								<p>Hello ${name},</p>
// 								<p>Congratulations! Your employee account has been successfully created at Finshelter.</p>
// 								<div class="credentials">
// 									<h3>Your Login Credentials:</h3>
// 									<p><strong>Username:</strong> ${username}</p>
// 									<p><strong>Temporary Password:</strong> ${password}</p>
// 									<p><strong>Designation:</strong> ${designation || 'Employee'}</p>
// 								</div>
// 								<p><strong>Important Security Note:</strong> Please log in and change your password immediately after your first login for security purposes.</p>
// 								<p>If you have any questions or need assistance, please don't hesitate to contact the admin team.</p>
// 								<p>Best regards,<br><strong>Finshelter Admin Team</strong></p>
// 							</div>
// 							<div class="footer">
...
const getAllUsers = async (req, res) => {
	try {
		// Fetch all users with referrer information populated
		const users = await User.find()
			.populate({
				path: "referredBy",
		...
		const usersWithDetails = await Promise.all(
			users.map(async (user) => {
				...
				// Return user details along with services, messages, and referrer info
				return { 
					...user.toObject(), 
					services, 
					messages,
					referrerName: user.referredBy ? user.referredBy.name : null,
					referrerEmail: user.referredBy ? user.referredBy.email : null
				};
			})
		);
...
const getDashboardData = async (req, res) => {
	try {
		...
		const transformedUsers = users.map((user) => ({
			...
			referrerName: user.referredBy ? user.referredBy.name : null,
			referrerEmail: user.referredBy ? user.referredBy.email : null,
			referrerCode: user.referredBy ? user.referredBy.referralCode : null,
		}));
