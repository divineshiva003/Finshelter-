...
// 		const userId = req.user?.userId || req.user?._id || req.body.userId || req.query.userId;
// 		if (!userId) {
...
// 		// Sync orders from Order model to user.services if they don't exist
// 		const Order = require('../models/orderModel');
// 		const orders = await Order.find({ customerId: userId.toString() });
		
// 		console.log("Orders from database count:", orders.length);
		
// 		// Create a Set of valid order IDs from the Order collection
// 		const validOrderIds = new Set(orders.map(order => order.orderId));
		
// 		// Remove services that don't have corresponding orders in database
// 		const originalServicesCount = user.services.length;
// 		user.services = user.services.filter(service => {
// 			// Keep only services that have a valid orderId in the Order collection
// 			const hasValidOrder = service.orderId && validOrderIds.has(service.orderId);
// 			if (!hasValidOrder && service.orderId) {
// 				console.log('Removing invalid service with orderId:', service.orderId);
// 			}
// 			return hasValidOrder;
// 		});
		
// 		if (user.services.length !== originalServicesCount) {
// 			console.log(`Removed ${originalServicesCount - user.services.length} invalid services`);
// 		}
		
// 		let servicesSynced = false;
// 		for (const order of orders) {
// 			// Check if this order already exists in user.services
// 			const existsInServices = user.services.some(s => s.orderId === order.orderId);
			
// 			if (!existsInServices) {
// 				console.log('Syncing order to user.services:', order.orderId);
// 				// Add this order to user's services
// 				user.services.push({
...
// 					status: order.orderStatus || "In Process",
...
// 		// Save user if services were cleaned up or synced
// 		if (servicesSynced || user.services.length !== originalServicesCount) {
// 			await user.save();
// 			console.log('Services cleaned and synced. Final count:', user.services.length);
// 		}
...
// 	try {
// 		const { name, email, mobile, password, leadSource, referralCode } = req.body;
...
// 		// Create new user (ID will be auto-generated by schema)
// 		const newUser = new User({
// 			name,
// 			email,
// 			mobile,
...
// 				await handleReferral(referralCode, newUser._id);
// 			} catch (refError) {
// 				console.error("Error handling referral:", refError);
...
// 	console.log("\n=== UPLOAD DOCUMENTS CONTROLLER REACHED ===");
...
// 		console.log('Upload request - userId:', userId, 'serviceId:', serviceId);
...
// 		console.log('User services:', user.services.map(s => ({
// 			serviceId: s.serviceId,
// 			serviceIdString: s.serviceId?.toString(),
// 			orderId: s.orderId
// 		})));
		
// 		let serviceIndex = user.services.findIndex(
// 			(s) => s.serviceId.toString() === serviceId
// 		);

// 		console.log('Service index found:', serviceIndex);

// 		// If not found in user.services, check if there's an order with this serviceId
// 		if (serviceIndex === -1) {
// 			console.log('Service not in user.services, checking Order model...');
// 			const Order = require('../models/orderModel');
...
// 			if (order) {
// 				console.log('Found order:', order.orderId, '- Adding to user.services');
// 				// Add this service to user's services array
// 				user.services.push({
// 					orderId: order.orderId,
// 					serviceId: serviceId,
// 					packageName: order.packageName,
...
// 					documents: []
// 				});
// 				serviceIndex = user.services.length - 1;
// 				console.log('Service added at index:', serviceIndex);
// 			} else {
// 				console.log('No order found either');
// 				return res
// 					.status(404)
// 					.json({ message: "Service not found in user's services or orders" });
// 			}
// 		}
...
// 		console.log('Documents uploaded successfully:', documentRecords.length);
...
// 			await sendGmail({
// 				to: user.email,
// 				subject: 'Login Verification OTP - Finshelter',
// 				html: `
// 					<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
...
// 		if (!service) {
// 			console.log('Service not found in database:', serviceId);
// 			return res.status(404).json({ message: "Service not found" });
// 		}
		
// 		// Check if service is active
// 		if (service.isActive === false) {
// 			console.log('Service exists but is inactive:', serviceId);
// 			return res.status(404).json({ message: "Service not available" });
// 		}
		
// 		console.log('Returning service:', service.name);
// 		res.json({ service });
// 	} catch (err) {
...
// 		console.log('Updating bank details for userId:', userId);
...
// 	try {
// 		const { name, email, googleId, avatarUrl, referralCode } = req.body;
...
// 		// If user exists, update Google ID and return user data with token
// 		if (user) {
...
// 		// Initialize wallet for the new Google user
// 		const newWallet = new Wallet({
...
// 		if (referralCode) {
// 			try {
// 				await handleReferral(referralCode, createdUser._id);
// 			} catch (refError) {
// 				console.error("Error handling referral:", refError);
// 				// Continue registration even if referral fails
// 			}
...
// 		// Create reset URL - use environment variable or default to production URL
...
const getCustomerDashboard = async (req, res) => {
	try {
		...
		// Fetch user with properly populated service details and referrer info
		const user = await User.findById(userId)
			.populate({
				path: "services.serviceId",
				select: "name description packages dueDate price salePrice gstRate",
...
const registerFlexiCustomer = async (req, res) => {
	...
	// Create new user (ID will be auto-generated by userModel as cus10001, cus10002, etc.)
	const newUser = new User({
			name,
			email,
			mobile,
...
const submitFeedback = async (req, res) => {
	try {
		...
		// Find the service in the user's services array - try both serviceId and orderId
		let serviceIndex = user.services.findIndex(
			(service) => service.serviceId && service.serviceId.toString() === serviceId
		);
		
		// If not found by serviceId, try to find by orderId
		if (serviceIndex === -1) {
			serviceIndex = user.services.findIndex(
				(service) => service.orderId === serviceId
			);
		}
...
const googleRegister = async (req, res) => {
	try {
		...
		// Handle referral code if provided in the request
		...
		// Create new customer (ID will be auto-generated by userModel as cus10001, cus10002, etc.)
		const newUser = new User({
			name,
			email,
			googleId,
