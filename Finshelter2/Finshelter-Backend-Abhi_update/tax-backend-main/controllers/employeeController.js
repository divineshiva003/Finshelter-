...
const getEmployeeDash = async (req, res) => {
	try {
		// Get employeeId from the authenticated user (from JWT token)
		const employeeId = req.user.userId || req.user._id;
		
		console.log('=== getEmployeeDash Debug ===');
		console.log('Employee ID from JWT:', employeeId);
		console.log('Full user from JWT:', req.user);
		...
		// Fetch basic employee information without complex populates to avoid errors
		const employeeInfo = await User.findById(employeeId)
			.select('-passwordHash -salt -resetPasswordToken -resetPasswordExpires -loginOTP -loginOTPExpires -googleId')
			.lean();
		...
		console.log('Employee found:', employeeInfo ? {
			...
			assignedCustomersCount: employeeInfo.assignedCustomers ? employeeInfo.assignedCustomers.length : 0
		} : null);
		...
		const metrics = {
			totalCustomers: employeeInfo.assignedCustomers ? employeeInfo.assignedCustomers.length : 0,
			activeCustomers: 0, // Will calculate this properly later
			completedServices: 0, // Will calculate this properly later
		};

		// Fetch status information
		const status = {
			isActive: employeeInfo.isActive || false,
			isProfileComplete: employeeInfo.isProfileComplete || false,
			lastLogin: employeeInfo.lastLogin || null,
		};
...
const updateServiceStatus = async (req, res) => {
	try {
		const { customerId, serviceId, status, employeeId } = req.body;
...
const getAssignedCustomers = async (req, res) => {
	const employeeId = req.user.userId; // Use authenticated user's ID
	
	console.log('=== DEBUG: getAssignedCustomers ===');
	console.log('Employee ID from JWT:', employeeId);
	console.log('Full user from JWT:', req.user);
	...
	try {
		...
		const employee = await User.findById(employeeId)
			.select("assignedCustomers L1EmpCode name email")
			.populate({
				path: "L1EmpCode",
				select: "name",
			});
		...
		console.log('Employee found:', employee ? {
			...
			assignedCustomersCount: employee.assignedCustomers ? employee.assignedCustomers.length : 0
		} : null);
		...
		if (!assignedCustomerIds || assignedCustomerIds.length === 0) {
			console.log('No customers assigned to employee');
			return res
				.status(200)
				.json({
		...
		console.log('Assigned customer IDs:', assignedCustomerIds.map(c => c._id || c));

		// Get manager name
		const managerName = employee.L1EmpCode ? employee.L1EmpCode.name : null;

		// Also check for any customers that have services assigned to this employee ID directly
		const directlyAssignedCustomers = await User.find({
		...
		console.log('Directly assigned customers (by employeeId in services):', directlyAssignedCustomers.map(c => ({
			id: c._id,
			name: c.name,
			servicesWithThisEmployee: c.services.filter(s => s.employeeId?.toString() === employeeId.toString()).map(s => s.orderId)
		})));
		...
		console.log('Customers from assignedCustomers array:', customers.map(c => ({
			id: c._id,
			name: c.name,
			servicesCount: c.services.length
		})));

		// Combine both approaches - customers from assigned list AND customers with direct employee assignment
		const allRelevantCustomers = [...customers];
		
		// Add customers that have services directly assigned to this employee but aren't in assignedCustomers
		for (const directCustomer of directlyAssignedCustomers) {
			const isAlreadyIncluded = allRelevantCustomers.some(c => c._id.toString() === directCustomer._id.toString());
			if (!isAlreadyIncluded) {
				allRelevantCustomers.push(directCustomer);
				console.log('Added directly assigned customer:', directCustomer.name, directCustomer._id);
			}
		}

		// Filter services for the current employee and include all required fields
		const filteredCustomers = allRelevantCustomers
			.map((customer) => {
