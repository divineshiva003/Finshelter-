		...
		const fetchServicesAndOrders = async () => {
			...
			if (response.data.orders && response.data.orders.length > 0) {
				console.log("First order rating debug:", {
					rating: response.data.orders[0]["Rating"],
					ratingType: typeof response.data.orders[0]["Rating"],
					rawFeedback: response.data.orders[0]["_rawFeedback"],
					feedbackRaw: response.data.orders[0]["Feedback Raw"]
				});
			}
				// const serviceIds = response.data.orders.map(order => order["Service ID"] || order.serviceId);
