...
```bash
curl -X GET "https://api.thefinshelter.com/api/admin/services" \
...
```bash
curl -X POST "https://api.thefinshelter.com/api/admin/login" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api.thefinshelter.com/api/employees/login" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api.thefinshelter.com/api/customers/user-login" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "https://api.thefinshelter.com/api/employees/queries?employeeId=YOUR_EMPLOYEE_ID_HERE"
```
**Expected**: No "Cannot read properties of undefined (reading '_id')" error

### 6. Test Employee Dashboard
```bash
curl -X GET "https://api.thefinshelter.com/api/employees/emdashboard?employeeId=YOUR_EMPLOYEE_ID_HERE"
```

### 7. Test Customer Dashboard  
```bash
curl -X GET "https://api.thefinshelter.com/api/customers/cdashboard?userId=YOUR_USER_ID_HERE"
```

### 8. Test Wallet Functionality
```bash
curl -X GET "https://api.thefinshelter.com/api/customers/wallet?userId=YOUR_USER_ID_HERE"
```

### 9. Test Contact Form (No Auth Required)
```bash
curl -X POST "https://api.thefinshelter.com/api/contact" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Test User",
...
### Collection Setup:
1. **Base URL**: `http://srv721182.hstgr.cloud:8000`
...
response=$(curl -s -o /dev/null -w "%{http_code}" "https://api.thefinshelter.com/api/admin/services")
...
response=$(curl -s -o /dev/null -w "%{http_code}" -X POST "https://api.thefinshelter.com/api/contact" \
  -H "Content-Type: application/json" \
